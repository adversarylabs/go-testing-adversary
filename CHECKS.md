...
| **Looks for** | Known Testify equality assertions, or manual comparisons that fail the Go test, around direct selector calls whose literal or keyed table inputs have at least two scalar values and whose expected result is consistently the same input boundary across changed test files in the package |
| **Stays quiet when** | Any changed package case mixes boundary positions or uses an interior winner; the input is a singleton; assertions only check membership; assertion provenance is unknown; a comparison only logs; the API explicitly promises first/last/stable order; the selector contract or literal oracle cannot be proven syntactically |
